...
时间复杂度: O(n) java contain是暴力搜索但是实际不会发生O(n*n)的情况。
...
scanner.nextLine()返回String，需要Integer.parseInt()转为整数
...
System.arraycopy用来处理数组复制最有效率的方法。ArrayList底层用的它。
